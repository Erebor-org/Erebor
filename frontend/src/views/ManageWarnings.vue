<template>
  <div class="min-h-screen">
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="text-center mb-10">
        <h1 class="text-4xl md:text-5xl font-serif font-bold brand-gradient-text mb-4">Avertissements</h1>
        <div class="w-24 h-1 rounded-full mx-auto" style="background-image: linear-gradient(90deg, var(--primary), var(--accent));"></div>
        <p class="text-theme-text-muted mt-4">Historique des avertissements donnés aux membres d'Erebor</p>
      </div>
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-4 md:justify-between">
          <div class="relative flex-1 md:max-w-md">
            ...
            <input
              type="text"
              v-model="searchQuery"
              placeholder="Rechercher un pseudo, une raison..."
          ...
          <div class="flex items-center gap-3 justify-between md:justify-end">
            <span class="text-sm text-theme-text-muted whitespace-nowrap">{{ displayWarnings.length }} avertissement{{ displayWarnings.length === 1 ? '' : 's' }}</span>
            <button
      ...
      <div class="glass-card rounded-2xl overflow-hidden">
        <div v-for="warning in displayWarnings" :key="warning.id" class="wr-row">
          <div class="wr-row-main">
            <span class="wr-avatar">
              <img :src="getClassIcon(warning.character.class)" :alt="`Classe ${warning.character.class}`" />
            </span>

            <router-link :to="`/warnings/${warning.character.id}/${warning.character.pseudo}`" class="wr-identity" title="Voir tous les avertissements de ce personnage">
              <span class="wr-pseudo">{{ warning.character.pseudo }}</span>
              <span class="wr-date">{{ formatDate(warning.createdAt) }}</span>
            </router-link>
        ...
        <!-- Empty State -->
        <div v-if="displayWarnings.length === 0" class="text-center py-16 text-theme-text-muted">
          ...
          <p class="text-lg font-medium mb-1">{{ warnings.length === 0 ? 'Aucun avertissement' : 'Aucun résultat' }}</p>
          <p class="text-sm">{{ warnings.length === 0 ? 'La liste est vide pour le moment' : 'Essayez une autre recherche' }}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  >
    <div class="glass-modal rounded-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
      ...
      <p class="text-sm text-theme-text-muted mb-5">Consigne un manquement au règlement pour un membre.</p>

      <div class="space-y-4">
        <div class="space-y-1.5">
          <label class="block text-sm font-medium text-theme-text">Personnage <span class="text-theme-error">*</span></label>
          <div v-if="newWarning.characterId" class="wr-picked">
            <img :src="selectedCharacterIcon" :alt="`Classe ${selectedCharacterName}`" class="wr-picked-icon" />
            <span class="flex-1 font-semibold text-theme-text">{{ selectedCharacterName }}</span>
            <button type="button" @click="clearSelectedCharacter" class="wr-picked-clear" title="Changer de personnage">
  ...
  <ConfirmModal
    ...
    :message="selectedWarning ? `Êtes-vous sûr de vouloir supprimer cet avertissement pour ${selectedWarning.character.pseudo} ?` : ''"
...
export default {
  ...
  data() {
    return {
      ...
      newWarning: { characterId: '', description: '', authorCharacterId: '' },
  ...
  methods: {
    ...
    selectCharacter(character) {
      ...
      this.selectedCharacterIcon = getClassIcon(character.class);
    ...
    async fetchCharacters() {
      try {
        ...
        this.$refs.notificationRef.showNotification('Erreur lors de la récupération des personnages', 'error');
      }
    },
    ...
    openAddWarningModal() {
      this.newWarning = { characterId: '', description: '', authorCharacterId: '' };
    ...
    async addWarning() {
      if (!this.newWarning.characterId || !this.newWarning.description.trim() || !this.newWarning.authorCharacterId) {
...
.wr-detail {
  padding: 0 1.25rem 1.1rem calc(2.1rem + 0.85rem);
}
.wr-desc-full {
  font-size: 0.85rem;
  color: var(--text);
...
/* ---------- Sélecteurs personnage / auteur (modale d'ajout) ---------- */
.wr-picked {
  display: flex;
  align-items: center;
  gap: 0.75rem;
